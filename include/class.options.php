...
class CptoOptionsInterface {
	...
	/**
	 * CptoOptionsInterface constructor.
	 */
				...
				<table class="form-table">
					<tbody>
						<tr valign="top">
							...
							<td>
								...
								foreach ( $post_types as $post_type_name ) {
									...
									/**
									 * Error reported from: WordPress.PHP.StrictInArray.MissingTrueStrict.
									 * Description: Not using strict comparison for in_array; supply true for third argument.
									 * Fix: Although optional, passing boolean true through thrid argument
									 * so that the mode is specified in which we want to perform the search
									 * since the array 'ignore_post_types' holds string values.
									 */
									...
									<p>
										...
										<select name="show_reorder_interfaces[<?php echo esc_html( $post_type_name ); ?>]">
											...
											<option value="hide"
												<?php
												if ( isset( $options['show_reorder_interfaces'][ $post_type_name ] ) && 'hide' === $options['show_reorder_interfaces'][ $post_type_name ] ) {
													echo ' selected="selected"';}
												?>
											>
						...
						<tr valign="top">
							...
							<td>
								<select id="role" name="capability">
									<option value="read" 
									<?php
									if ( isset( $options['capability'] ) && 'read' === $options['capability'] ) {
										echo 'selected="selected"';}
									?>
									>
									...
									<option value="edit_posts" 
										<?php
										if ( isset( $options['capability'] ) && 'edit_posts' === $options['capability'] ) {
											echo 'selected="selected"';}
										?>
									>
									...
									<?php
									if ( isset( $options['capability'] ) && 'publish_posts' === $options['capability'] ) {
										echo 'selected="selected"';}
									?>
									>
									...
									<option value="publish_pages" 
										<?php
										if ( isset( $options['capability'] ) && 'publish_pages' === $options['capability'] ) {
											echo 'selected="selected"';}
										?>
									>
									...
									<?php
									if ( ! isset( $options['capability'] ) || empty( $options['capability'] ) || ( isset( $options['capability'] ) && 'manage_options' === $options['capability'] ) ) {
										echo 'selected="selected"';}
									?>
									>
                                    ...
                                    // Words in hook names should be separated using underscores.
                                    // Expected:  'pto_admin_plugin_options_capability' , but found:  'pto/admin/plugin_options/capability'.
                                    do_action( 'pto/admin/plugin_options/capability' );
						...
						<tr valign="top">
							...
							<td>
								<p>
									<input type="checkbox" <?php esc_html( checked( '1', $options['autosort'] ) ); ?> id="autosort" value="1" name="autosort"> <?php esc_html_e( 'If checked, the plug-in automatically update the WordPress queries to use the new order (<b>No code update is necessarily</b>)', 'post-types-order' ); ?>
								</p>
								...
								<br />
								<a href=<?php esc_url( 'http://www.nsp-code.com/sample-code-on-how-to-apply-the-sort-for-post-types-order-plugin/' ); ?> target="_blank"><?php esc_html_e( 'Additional Description and Examples', 'post-types-order' ); ?></a></p>
						...
						<tr valign="top">
							<th scope="row" style="text-align: right;">
								<label for="navigation_sort_apply">
